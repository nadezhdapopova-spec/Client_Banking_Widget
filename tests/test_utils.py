...
def test_deserialize_info_valid(mock_file, mock_getsize):
...
def test_deserialize_info_empty_file(mock_getsize):
    result = deserialize_info("fake_path_test.json")
    assert result == []
...
def test_deserialize_info_not_list(mock_file, mock_getsize):
    result = deserialize_info("fake_path_test.json")
    assert result == []
    mock_getsize.assert_called_once()
...
def test_file_not_found_on_open(mock_file, mock_getsize):
