...
def get_mask_card_number(card_number: int | str) -> str:
    ...
    if len(card_symbols) != 16:
        masks_logger.error(f"Номер карты {card_number} имеет недопустимую длину.")
        raise ValueError("Номер карты должен содержать 16 цифр.")
    if not card_symbols.isdigit():
        masks_logger.error(f"Номер карты {card_number} имеет недопустимые символы.")
        raise ValueError("Номер карты должен содержать только цифры.")

    masks_logger.info(f"Маска для номера карты {card_number} создана.")
...
def get_mask_account(account_number: int | str) -> str:
    ...
    if len(account_symbols) < 4:
        masks_logger.error(f"Номер счёта {account_number} имеет недопустимую длину.")
        raise ValueError("Номер счёта должен содержать не менее 4 цифр.")

    if not account_symbols.isdigit():
        masks_logger.error(f"Номер счёта {account_number} имеет недопустимые символы.")
        raise ValueError("Номер счета должен содержать только цифры.")

    masks_logger.info(f"Маска для номера счёта {account_number} создана.")
